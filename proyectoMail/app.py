...
def validarUsuario():
    if request.method == "POST":
        ...
        usu = usu.replace("SELECT", "").replace("INSERT", "").replace(
...
def registrarUsuario():
    try:
        if request.method == "POST":
            ...
            nombre = nombre.replace("SELECT", "").replace("INSERT", "").replace(
                "DELETE", "").replace("UPDATE", "").replace("WHERE", "")
            ...
            if not util.isPasswordValid(passw):
                error = 'La contraseña debe contener al menos una minúscula, una mayúscula, un número, un caracter especial y 8 caracteres'
                return render_template('login.html', information=error)

            if not util.isEmailValid(email):
                error = 'El correo ' + email + ' no es valido, dijite un correo nuevo'
                return render_template('login.html', information=error)
            ...
            respuesta = controlador.registrar_usuario(
                nombre, email, passw2, codigo2)
...
def enviarMAIL():
    if request.method == "POST":
        ...
        emailDestino = emailDestino.replace("SELECT", "").replace(
            "INSERT", "").replace("DELETE", "").replace("UPDATE", "").replace("WHERE", "")
        asunto = request.form["asunto"]
        asunto = asunto.replace("SELECT", "").replace("INSERT", "").replace(
            "DELETE", "").replace("UPDATE", "").replace("WHERE", "")
        mensaje = request.form["mensaje"]
        mensaje = mensaje.replace("SELECT", "").replace("INSERT", "").replace(
            "DELETE", "").replace("UPDATE", "").replace("WHERE", "")
...
def actualizacionPassword():
    if request.method == "POST":
        ...
        pass1 = pass1.replace("SELECT", "").replace("INSERT", "").replace(
            "DELETE", "").replace("UPDATE", "").replace("WHERE", "")
        passw2 = pass1.encode()
        passw2 = hashlib.sha384(passw2).hexdigest()
...
def activarUsuario():
    if request.method == "POST":
        ...
        codigo = codigo.replace("SELECT", "").replace("INSERT", "").replace(
            "DELETE", "").replace("UPDATE", "").replace("WHERE", "")
